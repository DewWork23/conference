...
export default function NetworkingPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-5xl md:text-6xl font-black text-[#947843] leading-tight tracking-tight mb-2">Networking</h1>
        <p className="text-xl text-[#3E5C73] font-semibold tracking-wide">Connect with fellow attendees and presenters</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border-l-4 border-[#3E5C73]">
          ...
          <h3 className="text-lg font-bold text-[#3E5C73] tracking-wide mb-2">Attendee Directory</h3>
        ...
        <div className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border-l-4 border-[#3E5C73]">
          ...
          <h3 className="text-lg font-bold text-[#3E5C73] tracking-wide mb-2">Interest Groups</h3>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#947843] focus:border-transparent"
              />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredParticipants.map(participant => (
          <div key={participant.id} className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border-l-4 border-[#947843] hover:border-[#3E5C73]">
            <div className="flex items-start mb-4">
              <div className={`w-16 h-16 ${participant.type === 'presenter' ? 'bg-[#947843]' : 'bg-[#3E5C73]'} rounded-full flex items-center justify-center text-white text-xl font-bold flex-shrink-0`}>
                {participant.name.split(' ').map(n => n[0]).join('')}
              </div>
              <div className="ml-4 flex-1">
                <h3 className="text-lg font-bold text-[#3E5C73] tracking-wide mb-1">{participant.name}</h3>
                <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                  participant.type === 'presenter'
                    ? 'bg-[#947843] text-white'
                    : 'bg-[#3E5C73] text-white'
                }`}>
      ...
      {/* Interest Groups */}
      <div className="mt-12 bg-white p-6 rounded-lg shadow-md border-l-4 border-[#947843]">
        <h2 className="text-2xl font-bold text-[#947843] tracking-wide mb-6">Interest Groups</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={group.name} className="border-l-4 border-gray-200 rounded-lg p-4 hover:border-[#947843] transition-all duration-300 cursor-pointer shadow-md hover:shadow-xl bg-gray-50">
              <h3 className="text-lg font-bold text-[#3E5C73] tracking-wide mb-2">{group.name}</h3>
              <div className="flex items-center justify-between">
                <span className={`text-xs px-3 py-1 rounded-full ${group.color}`}>
                  {group.members} members
                </span>
                <button className="text-sm text-[#947843] hover:text-[#3E5C73] font-bold transition-colors">
                  Join
                </button>
              </div>
            </div>
